...
public class Movement : MonoBehaviour
{
    ...
    void OnCollisionEnter(Collision collision)
    {
        if(collision.gameObject.name == ("Seeker")){
             GameObject.Find("scoreText").GetComponent<UnityEngine.UI.Text>().text = "You Lost! Found : " + score + "/15"; 
            Time.timeScale = 0;
        }
        if(collision.gameObject.tag == ("HideUnit")){
            ...
            if (score == 15) {
                //Time.timeScale = 0;
                Application.LoadLevel("Level Main");
            GameObject.Find("scoreText").GetComponent<UnityEngine.UI.Text>().text = "You Won!";
            return;
            }
